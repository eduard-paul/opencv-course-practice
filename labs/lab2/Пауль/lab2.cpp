...
int main(int argc, char* argv[])
{
    ...
    {
        KeyPoint image = keypoints1[matches[matchInd].queryIdx];
        KeyPoint scene = keypoints2[matches[matchInd].trainIdx];

        float distance  = matches[matchInd].distance;

        float angle      = atan((center-image.pt).y/(center-image.pt).x);
        float delta      = CV_PI*(scene.angle-image.angle)/180.f;
        float sceneAngle = angle+delta;

        hgt.at<int>((cvRound(scene.pt.y+sin(sceneAngle)*distance))/splitSet, 
            (cvRound(scene.pt.x+cos(sceneAngle)*distance))/splitSet)++;
    }
    ...
    vector<Point2f> imagePoints(matches.size());
    vector<Point2f> scenePoints(matches.size());

    for (size_t match = 0; match < matches.size(); match++)
    {
        imagePoints[match] = keypoints1[matches[match].queryIdx].pt;
        scenePoints[match] = keypoints2[matches[match].trainIdx].pt;
    }
    Mat homography = findHomography(imagePoints, scenePoints, CV_RANSAC);

    Mat warp;
    warpPerspective(image, warp, homography, scene.size());

    imshow("Homography", warp);
    waitKey(0);
